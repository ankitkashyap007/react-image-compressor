export default function Upload({name, uploadFileInfo,handleChange, multiple,acceptFormats}){
        ...
        <div className="flex items-center justify-center w-full">
                    <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800  hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-indigo-100">
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p className="text-xs text-gray-800 dark:text-gray-600">{uploadFileInfo}</p>
                        </div>
                        <input id="dropzone-file" name={name} type="file" onChange={handleChange} className="sr-only" required accept={acceptFormats} multiple={multiple}/>
                    </label>
                </div>
    )
}