            ...
            <form>
                ...
                <Upload name="pdf" uploadFileInfo="PDFs (MAX. 10MB)" handleChange={handleChange} acceptFormats="application/pdf" multiple={true} />
                {
                    pdfsData[0] && <div className='flex flex-wrap pt-5 tracking-wide '> {Array.from(pdfsData).map(({ name, size }, index) => {
                        return <div key={name + index} className='inline-block justify-between bg-indigo-600 text-white rounded-md p-2 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 mb-4 mx-2 items-center overflow-hidden' draggable>
                            <span className='mx-1 inline-block'><b>Name: </b>{name}</span> <span className='rounded-md px-2 bg-indigo-800 text-sm inline-block'><b>Size: </b>{byteToMb(size)}</span>
                        </div>
                    })} </div>
                }
