...
export default function WpDetector() {
    ...
    return (<div className="px-5">
        ...
        <form onSubmit={handleSearch} className="px-5 flex flex-col border border-indigo-300 p-5 mb-10">

            <input className="rounded-md border border-solid border-2 px-2 mb-2 text-xl w-full" type="text" name="site" id="site-url" value={isSite} onChange={handleChange} placeholder="Website url" />
            <button className="bg-indigo-600 px-2 py-1 w-full font-bold text-xl text-white">Search</button>
        </form>
        <div className="w-full">
                ...
                <tbody>
                    {jsonData && jsonData.result && Object.entries(jsonData.result).map(([key, value]) => (
                        <tr className="bg-gray-100" key={key}>
                            ...
                            <td className="py-2 px-4 border-b">{Array.isArray(value) ? <ul>{value.map(item => <li>{item}</li>)}</ul> : typeof value === 'object' ? <ul className="list-disc">{Object.entries(value).map(([key, valuee]) => <li key={key}>{key}</li>)}</ul> : value}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            <table border="1">
                <thead>
                    <tr>
                        <th colSpan={2}>Theme Information</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    {jsonData && jsonData.themeInfo && Object.entries(jsonData.themeInfo).map(([key, value]) => (
                        <tr key={key}>
                            <td>{key}</td>
                            <td className="py-2 px-4 border-b">{Array.isArray(value) ? <ul>{value.map(item => <li>{item}</li>)}</ul> : typeof value === 'object' ? Object.entries(value).map(([key, valuee]) => <span className="m-1 px-2 py-1 border border-solid border-blue-500 inline-flex" key={key}>{valuee}</span>) : value}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            <table border="1">
                <thead>
                    <tr>   <th colSpan={2}>Plugin Information</th>
                    </tr>
                </thead>
                <tbody>
                            ...
                            {details ?
                                ...
                                Object.entries(details).map(([mainkey, value]) => (
                                    ...
                                    Array.isArray(value) ?
                                        ...
                                        <tr><td>{mainkey}</td><td className="py-2 px-4 border-b" > <ul>{value.map(item => <li>{item}</li>)}</ul></td></tr> :

                                        typeof value === 'object' ?
                                            mainkey === 'tags' ? <tr><td>{mainkey}</td><td>{Object.entries(value).map(([key, valuee]) => <li className="m-1 px-2 py-1 border border-solid border-blue-500 inline-flex" colSpan={2} key={key}>{valuee}</li>)}</td></tr>
                                                :
                                                Object.entries(value).map(([key, valuee]) =>
                                                    <tr>
                                                        <td className="py-2 px-4 border-b" key={key}>{mainkey}</td>
                                                        <td className="py-2 px-4 border-b" key={key}>{valuee}</td>
                                                    </tr>
                                                ) :
                                            <tr><td>{mainkey}</td><td>{value}</td> </tr>
